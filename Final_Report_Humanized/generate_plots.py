...
def plot_phase2_acc():
    ...
    values = [45.6, 50.2, 53.5, 38.1, 31.4, 54.7]
    
    fig, ax = plt.subplots(figsize=(11, 6))
    bars = ax.bar(labels, values, width=0.5, color=['#4C72B0', '#DD8452', '#55A868', '#C44E52', '#8172B3', '#937860'], edgecolor='black', alpha=0.9)
...
def plot_bias_combined():
    ...
    acc_values = [50.2, 38.1, 31.4]
    ds_values = [68.0, 45.0, 88.0]
...
def plot_lora_comparison():
    """2x2 subplot comparison: E2 Base vs E6 Dynamic LoRA, each metric on its own scale with delta arrows."""
    fig, axes = plt.subplots(2, 2, figsize=(12, 9))
    fig.suptitle('Impact of Dynamic LoRA Routing on Clinical Metrics', fontsize=16, fontweight='bold', y=0.98)
    
    metrics = [
        {'name': 'Diagnostic Accuracy', 'e2': 50.2, 'e6': 54.7, 'unit': '%', 'ymin': 30, 'ymax': 70},
        {'name': 'Diagnostic Stability (DS)', 'e2': 0.68, 'e6': 0.72, 'unit': '', 'ymin': 0.4, 'ymax': 1.0},
        {'name': 'Test Rationality (TR)', 'e2': 0.71, 'e6': 0.76, 'unit': '', 'ymin': 0.4, 'ymax': 1.0},
        {'name': 'Information Efficiency (IE)', 'e2': 0.98, 'e6': 1.02, 'unit': '', 'ymin': 0.6, 'ymax': 1.3},
    ]
    
    colors = ['#DD8452', '#937860']
    
    for idx, (ax, m) in enumerate(zip(axes.flat, metrics)):
        bars = ax.bar(['E2 (Base)', 'E6 (LoRA)'], [m['e2'], m['e6']], 
                      width=0.5, color=colors, edgecolor='black', alpha=0.9)
        
        ax.set_ylabel(m['name'], fontsize=11)
        ax.set_ylim(m['ymin'], m['ymax'])
        ax.set_title(m['name'], fontsize=12, fontweight='bold')
        
        # Value labels on bars
        for bar in bars:
            h = bar.get_height()
            fmt = '%.1f%s' if m['unit'] == '%' else '%.2f'
            label = (fmt % (h, m['unit'])) if m['unit'] == '%' else (fmt % h)
            ax.text(bar.get_x() + bar.get_width()/2., h + (m['ymax'] - m['ymin']) * 0.02,
    ...
    plt.savefig('Pictures/lora_comparison.pdf', bbox_inches='tight', dpi=150)
    plt.close()
