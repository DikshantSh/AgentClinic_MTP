...
To address this evaluation gap, the multi-agent LLM simulation paradigm was adopted and substantially extended. The work evolved in two phases.

\textbf{Initial Framework:} A functional multi-agent clinical simulator governed by an ad-hoc \texttt{while}-loop was initially built. Several open-source LLMs (Mistral-7B, Microsoft BioGPT, JSL-Med, and Apollo) were benchmarked, and baseline metrics for diagnostic accuracy and bias susceptibility were established. This initial approach, however, revealed serious architectural limitations---most notably non-deterministic state management and a vulnerability termed \textit{Lexical Leakage}---stemming from the use of homogeneous model instances across all agent roles.

\textbf{Enhanced Framework:} To address these shortcomings, the system was re-architected into \textbf{the enhanced AgentClinic framework}. The orchestration layer was redesigned as a LangGraph Directed Cyclic Graph (DCG) state machine, 4-bit NF4 double quantisation was introduced to support heterogeneous multi-engine configurations on a single GPU, and mathematical process-oriented metrics were formalised around a novel latent metacognitive reflection node.
