...
Key Features (v2.1 over v2.0):
    ...
    - ClinicalTrajectoryEvaluator integration (Stability, Rationality, Efficiency)
     ...
     ./test_venv/bin/python agentic_clinic_future_scope.py --doctor_llm HF_mistralai/Mistral-7B-Instruct-v0.2 --patient_llm HF_mistralai/Mistral-7B-Instruct-v0.2 --measurement_llm HF_mistralai/Mistral-7B-Instruct-v0.2 --moderator_llm HF_mistralai/Mistral-7B-Instruct-v0.2 --doctor_model_id mistralai/Mistral-7B-Instruct-v0.2 --patient_model_id mistralai/Mistral-7B-Instruct-v0.2 --num_scenarios 1
"""
...
class LocalServerLLMWrapper:
    ...
    def __init__(self, model_id: str, quantize: bool = True):
        ...
        # Force execution onto GPU exclusively instead of allowing CPU offloading
        load_kwargs = {"device_map": "cuda", "trust_remote_code": True}
        ...
        # --- FUTURE WORK: Dynamic LoRA Loading ---
        self.use_dynamic_lora = False
        if PEFT_AVAILABLE:
            # Placeholder paths for future fine-tuned LoRA checkpoints
            reasoning_lora_path = "adapters/reasoning_lora" 
            dialogue_lora_path = "adapters/dialogue_lora" 
            
            if os.path.exists(reasoning_lora_path) and os.path.exists(dialogue_lora_path):
                try:
                    print("[LocalServerLLMWrapper] Initializing PEFT architecture...")
                    self.model = PeftModel.from_pretrained(self.model, reasoning_lora_path, adapter_name="reasoning")
                    self.model.load_adapter(dialogue_lora_path, adapter_name="dialogue")
                    self.use_dynamic_lora = True
                    print("[LocalServerLLMWrapper] Successfully loaded dynamic PEFT adapters: routing enabled.")
                except Exception as e:
                    print(f"[LocalServerLLMWrapper] Warning: Could not load LoRA adapters: {e}")
            else:
                print("[LocalServerLLMWrapper] Dynamic LoRA adapters (reasoning/dialogue) not found at placeholder paths. Running base model.")
...
class DoctorAgent:
    ...
    def reflect_metacognition(self, latest_input, model_class) -> List[str]:
        ...
        # Switch to reasoning adapter if available (FUTURE WORK)
        if hasattr(model_class, 'use_dynamic_lora') and model_class.use_dynamic_lora:
            model_class.model.set_adapter("reasoning")
    ...
    def inference_doctor(self, question, model_class, is_final=False) -> str:
        ...
        # Switch to dialogue adapter if available (FUTURE WORK)
        if hasattr(model_class, 'use_dynamic_lora') and model_class.use_dynamic_lora:
            model_class.model.set_adapter("dialogue")
    ...
    def _force_final_diagnosis(self, question, model_class) -> str:
        ...
        # Ensure dialogue adapter is used for final diagnosis generation (FUTURE WORK)
        if hasattr(model_class, 'use_dynamic_lora') and model_class.use_dynamic_lora:
            model_class.model.set_adapter("dialogue")
